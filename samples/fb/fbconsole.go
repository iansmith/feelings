...
type FBResolution int

const (
	FBResolution1024x768  FBResolution = 1
	FBResolution1920x1200 FBResolution = 2
)

type FBConsole struct {
	info     *videocore.FrameBufferInfo
	font     *PCScreenFont
	virtualY uint32
	currentX uint32
	currentY uint32
...
func NewFBConsole(data *videocore.FrameBufferInfo, font *PCScreenFont) *FBConsole {
...
func (f *FBConsole) incrementY() {
	...
	if f.currentY == f.maxY {
		//if !videocore.SetVirtualOffset(0, 100*(f.virtualY+f.font.Height)) {
		//	panic("cant scroll")
		//}
